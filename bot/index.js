...
client.on('interactionCreate', async interaction =>{
    ...
    let parseField = interaction.customId.split("_");
    let row = parseField[1]
    let col = parseField[2]
    if(tictactoe_state[row][col]!=EMPTY)
    {
        interaction.update({
    ...
    {
        interaction.update({
            content:"The game result in a draw!",
            components: []
        })
        return;
    }
...
client.on('interactionCreate', async interaction =>{
    ...
    {    tictactoe_state = [
        ...
        await interaction.reply({content: 'Playing game of tictactoe', components: makeGrid() });
    }
})
